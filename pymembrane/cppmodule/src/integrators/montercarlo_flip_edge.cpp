...
int MonteCarloIntegratorFlipEdge::integrate(void)
{
    ...
    // for (int ii = 0; ii < 10; ii++)
    {
        ...
        if (freezed_edge[_system.edges[edge_index].type] == false)
        {
            ...
            if (this->edge_flip(edge_index, true) == true)
            {
                ...
                accepted_moves++;
                // std::cout<<_system.edges[edge_index].v0 <<" "<< _system.edges[edge_index].v1<<" "<<_system.edges[edge_index].v2<<" "<<_system.edges[edge_index].v3<<std::endl;
                // std::cout<<"delE:"<<delE<<" P:"<<exp(-delE / get_temperature())<<std::endl;
                if (!(delE < 0.0))
                {
    ...
    // std::cout<<"accepted:"<<accepted_moves<<std::endl;
