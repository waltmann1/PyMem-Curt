...
/** @defgroup integrators Vertex Integrators
 *  @brief MonteCarloIntegrator abstracts definitions
 *  @{
 */
#include <memory>
#include <math.h>
 ...
 * @brief Abstract class for vertex integrator
 */
class MonteCarloIntegrator
{
public:
  ...
  void update_vertex_normal(const int &vertex_index)
  {
    for (const auto& f : _potentials)
      f.second->update_vertex_normal(vertex_index);
  }
