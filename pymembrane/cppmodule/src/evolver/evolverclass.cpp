...
{
    std::transform(_system.vertices.begin(), _system.vertices.end(), _system.vertices.begin(), host::reset_vertex_forces());
}

//compute
void EvolverClass::reset_mesh_energy(void)
{
    std::transform(_system.vertices.begin(), _system.vertices.end(), _system.vertices.begin(), host::reset_vertex_energy());
    std::transform(_system.edges.begin(), _system.edges.end(), _system.edges.begin(), host::reset_edge_energy());
    std::transform(_system.faces.begin(), _system.faces.end(), _system.faces.begin(), host::reset_face_energy());
}
...
void EvolverClass::reset_mesh_stresses(void)
{
    if (has_face_forces == true)
        std::transform(_system.stress_group_faces.begin(), _system.stress_group_faces.end(), _system.stress_group_faces.begin(), host::reset_tensor<realTensor>());
    if (has_vertex_forces == true)
        std::transform(_system.stress_group_vertices.begin(), _system.stress_group_vertices.end(), _system.stress_group_vertices.begin(), host::reset_tensor<realTensor>());
    if (has_edge_forces == true)
        std::transform(_system.stress_group_edges.begin(), _system.stress_group_edges.end(), _system.stress_group_edges.begin(), host::reset_tensor<realTensor>());
}

void EvolverClass::reset_mesh_atom_stresses(void)
{
    std::transform(_system.stress_virial_atom.begin(), _system.stress_virial_atom.end(), _system.stress_virial_atom.begin(), host::reset_tensor<realTensor>());
}
...
{
    if (name.compare("Mesh>Brownian>Positions") == 0)
    {
...
void EvolverClass::evolve_mesh_md(void)
{
...
void EvolverClass::add_constraints(const std::string &name, std::map<std::string, std::string> &parameters)
{
    if (name.compare("Mesh>Volume") == 0)
    {
        ...
        {
          if (item.first.compare("max_iter") == 0) max_iter_mesh_contraint = util::from_string_int(item.second);
        }
