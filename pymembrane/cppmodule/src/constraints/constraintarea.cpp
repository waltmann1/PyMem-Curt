    ...
    for (int vertex_index = 0; vertex_index < _system.Numvertices; vertex_index++)
    {
        ...
        do
        {
            ...
            if (face_index != -1) // Remember -1 is the virtual face outside of the mesh
            {
                ...
                // compute
                host::compute_form_factor_triangle(g_now, _system.vertices[vf[0]].r, _system.vertices[vf[1]].r, _system.vertices[vf[2]].r);
                vf[2] = _system.faces[face_index].v3;
                A_T = host::compute_area_triangle_from_metric(g_now);
                m_value += A_T;
                vsub(r12, _system.vertices[vf[1]].r, _system.vertices[vf[0]].r);
                vsub(r13, _system.vertices[vf[2]].r, _system.vertices[vf[0]].r);
